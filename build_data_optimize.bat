...
for %%i in (*.pine) do (
	echo %%i| findstr "monster hero font hud_castle" 1>nul 
	IF errorlevel 1 (
		...
		%ANIM_EXPORT% 	-i %%i		-o %%~ni.bpine  QUALITY_TRUE_COLOR	EXPORT_AS_3D_TEXTURE	EXPORT_FMODULE_OPACITY USE_MULTI_MODULE_TYPE EXPORT_PALETTE_COLOR PIXEL_16BITS
	)
)
copy *.h %SRC_DIR_H%
