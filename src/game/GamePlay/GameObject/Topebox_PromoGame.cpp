...
void TopeBox_PromoGame::InitGamePromo(float x, float y, int w, int h, enumTypeTopeboxAds type_ads, enumTopeboxAdsPos pos)
{
	TDGame *_game = (TDGame*)GAME()->_current_game;
	_gp = G();
	_is_ready = false;
	_renderx = 0;
	_rendery = 0;
	...
	is_rendered = false;
	_pos_topebox_ads = pos;

	//Kha update point promo
	promo_point_o.X = _game->_point_o.X; 
	promo_point_o.Y = _game->_point_o.Y; 
}
...
{
	//PDEBUG("\n ######render topebox ads: x:%f -- y:%f -- _is_need_reload:%d -- _reGetTopeboxInfo:%d", x, y, _is_need_reload, _reGetTopeboxInfo);
	...
	if (typeAds == enumTypeTopeboxAds::TOPEBOX_ADS_ICON)
	{
		...
		G()->ClearScale();
		GET_SPRITE(SPRITEID_FONT_TOPEBOX_ADS)->_char_spacing = 2;
		...
		GET_SPRITE(SPRITEID_FONT_TOPEBOX_ADS)->DrawAnsiTextFormat(G(), _renderx + 55 + _icon_w - 70, _rendery + _icon_h * 1.3f + 25, _HCENTER | _VCENTER, "%s", _des);
		//G()->ClearScale();
	...
	if (typeAds == enumTypeTopeboxAds::TOPEBOX_ADS_BANNER)
	{
		...
		{
			_renderx = promo_point_o.X + k_SCREEN_WIDTH / 2 - _icon_w / 2;
		}
		else
		{
			_renderx = promo_point_o.X;
		}

		if (_icon_h < k_SCREEN_HEIGHT)
		{
			_rendery = promo_point_o.Y + k_SCREEN_HEIGHT / 2 - _icon_h / 2;
		}
		else
		{
			_rendery = promo_point_o.Y;
		}
...
bool TopeBox_PromoGame::UpdateTouchGamePromo()
{
	...
	{
		if (typeAds == enumTypeTopeboxAds::TOPEBOX_ADS_ICON && is_rendered)
		{
...
void TopeBox_PromoGame::SetDataAds(char game_name[], char link_store[], char link_icon[], char game_id[]) {
	sprintf(_game_name, "%s", game_name);
	sprintf(_link_store, "%s", link_store);
	sprintf(_link_icon, "%s", link_icon);
	sprintf(_game_id, "%s", game_id);
	sprintf(_des, "%s", game_name);
}
...
void TopeBox_PromoGame::SetTopeBoxPromoGameInforFromConfig() {
	...
	char sv_order[64];
	PDEBUG("\n #######################SetTopeBoxPromoGameInforFromConfig");
	PDEBUG("\n ##### _game_name: %s", _game_name);
	PDEBUG("\n ##### _link_store: %s", _link_store);
	PDEBUG("\n ##### _link_icon: %s", _link_icon);
	sprintf(_des, "%s", _game_name);

	_rev = 0;
#if defined (ANDROID_OS)
	if (Device::RemoteConfigGetInteger("GamePromoteVersion") != NULL)
	{
		_rev = Device::RemoteConfigGetInteger("GamePromoteVersion");
	}
	else {
		_rev = _game->_user._game_promo_revision;
	}
#endif
	PDEBUG("\n revison:%d -- game->_user._game_promo_revision:%d", _rev, _game->_user._game_promo_revision);
	PDEBUG("\n _link_icon:%s -- game->_user._game_promo_icon_id:%s -- strcmp(game->_user._game_promo_icon_id, _link_icon):%d", _link_icon, _game->_user._game_promo_icon_id, strcmp(_game->_user._game_promo_icon_id, _link_icon));
	if (_rev != _game->_user._game_promo_revision && _rev >= 0)
	{
		PDEBUG("\n ##### update GameInfo: %d", _rev);
		_game->_user._game_promo_revision = _rev;
		_game->_user._game_promo_has_show = true;
		_game->_user.UserDataSave();
	...
	else
	{
		...
		GAME()->is_config_downloaded = true;
...
void TopeBox_PromoGame::SetTopeBoxPromoGameInfo(string dataContent)
{
	...
	if (revi != _game->_user._game_promo_revision
		|| strcmp(_game->_user._game_promo_icon_id, _link_icon) != 0
		|| _game->_user._game_promo_order >= order)
	{
		PDEBUG("\n ##### update GameInfo: %d", revi);
		_game->_user._game_promo_revision = revi;
		_game->_user._game_promo_has_show = true;
		_game->_user._game_promo_order = 0;
		sprintf(_game->_user._game_promo_icon_id, "%s", _link_icon);
		_game->_user.SaveToBuffer();
		DownloadImageURL();
	}
	else
	{
...
void TopeBox_PromoGame::LoadImageToLocal() {
	...
	if (_game_icon.CreateImageFromPNGinDocument(G(), GAME_ICON_FILE_NAME) > 0)
	{
		...
		if (typeAds == enumTypeTopeboxAds::TOPEBOX_ADS_BANNER)
		{
			...
			float scaleX, scaleY;
			if (_game->_screen_width > _game->_screen_height)
			{
				scaleX = _game->_screen_width / 960.0f;
				scaleY = _game->_screen_height / 640.0f;
			}
			else
			{
				scaleX = _game->_screen_width / 640.0f;
				scaleY = _game->_screen_height / 960.0f;
		...
		PDEBUG("\n ##### create IMG success \n");
		PDEBUG("\n winW:%f -- winH:%f", _game->_screen_width, _game->_screen_height);
		GAME()->is_config_downloaded = true;
		if (typeAds == TopeBox_PromoGame::enumTypeTopeboxAds::TOPEBOX_ADS_BANNER)
		{
			show_banner_topebox_active = true;
...
void TopeBox_PromoGame::GetTopeboxAdsInfoRemoteConfig() {
	PDEBUG("\n ##################################GetTopeboxAdsInfoRemoteConfig  => typeAds:%d", typeAds);
		...
		if (Device::RemoteConfigGetString("GamePromoting") != NULL)
		{
			...
			GAME()->time_delay_get_config = TIME_REMOTE_CONFIG;
		}
#endif
}
...
void TopeBox_PromoGame::GenerateDeviceIDString()
{
	...
	_device_id[count] = '\0';
	sprintf(_game->_user._game_promo_deviceID, "%s", _device_id);
}
