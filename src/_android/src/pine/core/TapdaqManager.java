...
import pine.core.Actions.ActionTapdaqRewardedVideoAdsShow;
import pine.core.Actions.ActionTapdaqRewardedVideoAdsShowArg;
import pine.core.Tapdaq.InterstitialListener;
import pine.core.Tapdaq.RewardedVideoListener;
import pine.core.Tapdaq.TapdaqInitListener;
...
public class TapdaqManager{
	...
	static public void init(Activity activity){
		...
		enabledPlacements.add(TapdaqPlacement.createPlacement(Arrays.asList(CreativeType.INTERSTITIAL_LANDSCAPE), "default"));
		enabledPlacements.add(TapdaqPlacement.createPlacement(Arrays.asList(CreativeType.INTERSTITIAL_LANDSCAPE, CreativeType.INTERSTITIAL_PORTRAIT), "mainMenu"));
		enabledPlacements.add(TapdaqPlacement.createPlacement(Arrays.asList(CreativeType.REWARDED_VIDEO_INTERSTITIAL), "default"));
	...
	static public boolean isPrepareRewardedVideoCompleted(){
		return Tapdaq.getInstance().isRewardedVideoReady(CurrentActivity, "rewardVideo");
	}
	...
	{
		Log.i("DEBUG", " Reward id " + Tapdaq.getInstance().getRewardId("rewardVideo"));
		if (!needWait) {
			if (!success) {
				LoadRewardedVideo();
			}
