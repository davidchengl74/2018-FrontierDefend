		...
		switch(stream.ServiceCode){
							...
							public void onDone(){
								...
								((Activity)AppContext.CurrentContext).runOnUiThread(new Runnable() {
									public void run() {
										...
										//update add facebook token
										String token = FacebookManager.GetCurrentAccessToken();
										data_size += 2 + 2* token.length();
										Log.i("DEBUG",token);
			...
			case ServiceCode.UnityAdsPrepareVideo: {
				try {
					((Activity) AppContext.CurrentContext).runOnUiThread(new Runnable() {
						public void run() {
							Log.i("DEBUG", "Java => UnityAdsPrepareVideo");
			...
			case ServiceCode.UnityAdsShowAds: {
				try {
					((Activity) AppContext.CurrentContext).runOnUiThread(new Runnable() {
						public void run() {
							if (UnityAds.isReady("mainMenu")){
								UnityAds.show((Activity) AppContext.CurrentContext, "mainMenu");
							}
							else{
