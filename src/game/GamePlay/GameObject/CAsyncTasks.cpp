...
void CAsyncTasks::EndCommand(int i)
{
	...
	_command[i]._typeCommand = enumTypeCommand::COMMAND_TYPE_NONE;
	_command[i]._stateCommand = enumStateCommand::COMMAND_STATE_NONE;
	if (GAME()->_processing_view_type == PROCESSING_VIEW_TYPE_SYNC_SAVE)
	{
...
void CAsyncTasks::UpdateStateProcessing(int i)
{
	...
	case CAsyncTasks::COMMAND_TYPE_ASYN_UPDATE_CLOUND_SAVE:
		...
		if (_svc_update_cloud_save.Status == PineServiceResult::Responsed)
		{
			...
			else if (_svc_update_cloud_save.ResponseStatus == 0)
			{
				...
				if (_is_force_sync_save_to_cloud)
				{
					_is_force_sync_save_to_cloud = false;
				}
				else
				{
	...
	case CAsyncTasks::COMMAND_TYPE_ASYN_SHARE_SCREEN_SHOT_INTENT:
		...
		{
			PokerFlat* game = (PokerFlat*)(GAME()->_current_game);
	...
	case CAsyncTasks::COMMAND_TYPE_ASYN_FIREBASE_DOWNLOAD_CONFIG:
	{
		...
		{
			OS_AsynFirebaseDowloadConfig(&_svc_firebase_download_config, GAME()->_topebox_game._link_icon, FIREBASE_BUTKET_URL, GAME_ICON_FILE_NAME);
		}
				...
				if (_firebase_file_type == enumFirebaseDownloadFileType::FIREBASE_TYPE_CONFIG)
				{
					...
					if (buffsv != NULL) {
						...
						for (int i = count; i < strlen(server_buff); i++) {
							if (server_buff[i] == 10)
							{
								GAME()->_topebox_game._game_name[_count] = '\0';
								count = i + 1;
								break;

							}
							GAME()->_topebox_game._game_name[_count] = (char)server_buff[i];
							_count++;
						...
						for (int i = count; i < strlen(server_buff); i++) {
							if (server_buff[i] == 10)
							{
								GAME()->_topebox_game._type_ads[_count] = '\0';
								count = i + 1;
								break;

							}
							GAME()->_topebox_game._type_ads[_count] = (char)server_buff[i];
							_count++;
						...
						for (int i = count; i < strlen(server_buff); i++) {
							if (server_buff[i] == 10)
							{
								GAME()->_topebox_game._link_store[_count] = '\0';
								count = i + 1;
								break;

							}
							GAME()->_topebox_game._link_store[_count] = (char)server_buff[i];
							_count++;
						...
						for (int i = count; i < strlen(server_buff); i++) {
							if (server_buff[i] == 10)
							{
								GAME()->_topebox_game._link_icon[_count] = '\0';
								count = i + 1;
								break;

							}
							GAME()->_topebox_game._link_icon[_count] = (char)server_buff[i];
							_count++;

						}
						PDEBUG("\n ===============%s \n+++ %s \n+++ %s \n+++ %s", GAME()->_topebox_game._game_name, GAME()->_topebox_game._type_ads, GAME()->_topebox_game._link_store, GAME()->_topebox_game._link_icon);
						
						//sprintf(GAME()->global_link_store, "%s", GAME()->_topebox_game._link_store);

						if (strcmp(GAME()->_topebox_game._type_ads, "banner") == 0)
						{
							GAME()->_topebox_game.typeAds = TopeBox_PromoGame::enumTypeTopeboxAds::TOPEBOX_ADS_BANNER;
							//GAME()->global_typeads = TopeBox_PromoGame::enumTypeTopeboxAds::TOPEBOX_ADS_BANNER;
						}
						else {
							GAME()->_topebox_game.typeAds = TopeBox_PromoGame::enumTypeTopeboxAds::TOPEBOX_ADS_ICON;
							//GAME()->global_typeads = TopeBox_PromoGame::enumTypeTopeboxAds::TOPEBOX_ADS_ICON;
						}
