...
public class FacebookManager
{
		...
		LoginManager.getInstance().registerCallback(_callback_mng,
			new FacebookCallback<LoginResult>() {
				...
				public void onSuccess(LoginResult loginResult) {
					...
					java.util.Set<String> recent_permission = loginResult.getRecentlyGrantedPermissions();
					boolean read_permission = false;
					boolean publish_permission = false;
					for(String permission : recent_permission)
					{
						if(permission.equals("public_profile"))
						{
							read_permission = true;
						}
						else if(permission.equals("publish_actions"))
						{
							publish_permission = true;
						}
					}

					boolean need_request_publish_permission = false;
					if(read_permission && !publish_permission && AppConfig.UseFacebookPublishPermission)
					{
						need_request_publish_permission = true;
					}


					//get infomation
					_profile = Profile.getCurrentProfile();
					_token = loginResult.getAccessToken();
					...
					if(_profile != null)
					{
						...
						for(int s = 0; s< _waitting_arg.size(); s++)
						{
							...
							else if(arg instanceof ActionFacebookRequestPostPermissionArg)
							{
								tmp.add(arg);
							}
						}
					...
					if(need_request_publish_permission)
					{
						ActionFacebookRequestPostPermissionArg arg = new ActionFacebookRequestPostPermissionArg();
						requestPostPermission(arg);
					}
				}
	...
	public static void requestPostPermission(ActionFacebookRequestPostPermissionArg arg)
	{
		_waitting_arg.add(arg);
		((Activity)AppContext.CurrentContext).runOnUiThread(new Runnable() {
			public void run() 
			{
				LoginManager.getInstance().logInWithPublishPermissions((Activity)AppContext.CurrentContext, Arrays.asList("publish_actions"));
			}
		});
	}
}