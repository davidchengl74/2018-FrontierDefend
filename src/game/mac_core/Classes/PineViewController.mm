...
extern "C" void OS_GoogleAnalyticsLogContentsWithType(const char *title,const char* decs,const char* type)
{
    ...
    NSString *sdecs=[NSString stringWithFormat:@"%s",decs];
    NSString *stype=[NSString stringWithFormat:@"%s",type];
    NSLog(@"\n ios logContent: title:%@ --- desc:%@ --- type:%@",stitle,sdecs,stype);
    [FIRAnalytics logEventWithName:stype
                        parameters:@{
                                     kFIRParameterItemID:[NSString stringWithFormat:@"id-%@", stitle],
                                     kFIRParameterItemName:stitle,
                                     kFIRParameterContentType:stitle,
                                     @"full_text":sdecs
                                     }];
}
